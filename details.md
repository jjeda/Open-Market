 ...
 또한, 물리적인 성능 향상을 위해 인메모리 기반의 솔루션을 통해 빠른성능에 대응하려고 했고, 그중에서도 오픈소스이며, 스프링의 PSA로 인하여 사용이 쉬운 Redis 를 선택하였습니다. 부가적으로 Redis는 RDB 및 AOF 기능을 통해 인메모리 데이터베이스에서도 데이터를 유지할 수 있었고 이는 서버의 장애로인해 메인 메모리 데이터가 손실되더라도 복구할 수 있는 장점 등을 활용할 수 있었습니다.
